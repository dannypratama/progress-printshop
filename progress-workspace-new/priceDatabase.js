...
window.addInvoiceItem = function () {
  ...
  wrap.insertAdjacentHTML(
    "beforeend",
    `
<div class="invoice-product-group">
...
window.removeInvoiceItem = function (button) {
  ...
  // kalau semua size dalam produk sudah terhapus,
  // hapus sekalian product group
  if (group && group.querySelectorAll(".invoice-item-row").length === 0) {
    group.remove();
  }

  updateInvoiceTotals();

  if (fallbackTarget && document.body.contains(fallbackTarget)) {
...
window.updateInvoiceTotals = function (manualPrice = false) {
  ...
  document.querySelectorAll(".invoice-item-row").forEach((row) => {
    ...
    const product = group
      ? group.querySelector(".invoice-product")?.value || ""
      : row.querySelector(".invoice-product")?.value || "";
    ...
    /*
      kalau user edit harga manual,
      kunci harga supaya tidak dioverwrite auto
    */
    if (manualPrice) {
      row.dataset.priceMode = "manual";
    }

    /*
      AUTO PRICE
      hanya jalan kalau mode masih auto
    */
    if (row.dataset.priceMode === "auto" && priceInput) {
      let autoPrice = 0;

      /*
        SESUAIKAN BAGIAN INI
        DENGAN RULE HARGA LAMA LU
      */
  ...
  if (subtotalEl) {
    subtotalEl.value = "Rp" + subtotal.toLocaleString("id-ID");
  }
};

document.addEventListener("input", function (e) {
  if (e.target.classList.contains("invoice-price")) {
    const row = e.target.closest(".invoice-item-row");

    row.dataset.priceMode = "manual";

    row.dataset.manualPrice = parseFloat(e.target.value) || 0;
...
document.addEventListener("input", function (e) {
  if (e.target.id === "invoice-price-adjustment") {
    document
      .querySelectorAll("#invoice-items .invoice-item-row")
      .forEach((row) => {
        // HANYA AUTO YANG PINDAH KE ADJUST

        if (row.dataset.priceMode === "auto") {
          row.dataset.priceMode = "adjust";
        }
      });
