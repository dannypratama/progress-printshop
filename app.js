...
async function saveDesignOrder() {
  ...
  try {
    if (app.editDesignId) {
      ...
      showToast("Desain berhasil diperbarui");
    } else {
...
window.updateInvoiceTotals = function () {
  ...
  document.querySelectorAll(".invoice-item-row").forEach((row) => {
    ...
    row.querySelector(".invoice-total").value = formatRupiah(total);

    subtotal += total;
  });

  document.getElementById("invoice-subtotal").value = formatRupiah(subtotal);

  const discount =
    Number(document.getElementById("invoice-discount").value) || 0;

  document.getElementById("invoice-total").value = formatRupiah(
    subtotal - discount,
  );
};
...
window.saveInvoice = async function () {
  ...
  if (!customer) {
    return showToast("Customer wajib diisi", "error");
  }

  const items = collectInvoiceItems();

  if (!items.length) {
    return showToast("Minimal 1 item invoice", "error");
  }
...
window.autoFitInvoice = function () {
  ...
  // A4: 794px x 1123px
  const availableHeight = canvas.clientHeight - 40;
...
window.generateInvoiceHTML = function (invoiceData) {
  ...
  for (let i = 0; i < emptyRowCount; i++) {
    tableRows += `<tr><td colspan="6" class="empty-row-td" style="height: 38px;"></td></tr>`;
  }
...
// ============================================================
// 3. FIX: BUKA MODAL TAMBAH INVOICE
// ============================================================
window.openAddInvoiceModal = async function () {
  await prepareInvoiceModal();
  addInvoiceItem(); // Tambah 1 item kosong buat user
  openModal("modal-invoice");
};

// ============================================================
// 4. FIX: SAVE INVOICE - HAPUS DUPLIKAT & PERBAIKI LOGIKA
// ============================================================
window.saveInvoice = async function () {
  const customer = document.getElementById("invoice-customer").value.trim();
...
// ============================================================
// 5. FIX: OPEN EDIT INVOICE - LENGKAP DENGAN MODAL TITLE UPDATE
// ============================================================
window.openEditInvoice = function (id) {
  ...
  // Update modal title ke mode EDIT
  const modalTitle = document.querySelector("#modal-invoice .modal-title");
  if (modalTitle) {
  ...
  (invoice.items || []).forEach((item) => {
    ...
    row.querySelector(".invoice-product").value = item.product || "";
...
// Trigger saat tombol "Tambah Invoice" di klik untuk membuka Modal Form
async function openAddInvoiceModal() {
  const form = document.getElementById("invoiceForm");
  form.reset(); // Reset form lama

  // Set otomatis nomor invoice ke input field (readonly agar user tidak merusak sequence)
  const nextInvNumber = await generateNextInvoiceNumber();
  const invInput = document.getElementById("inputInvoiceNumber");
  if (invInput) {
    invInput.value = nextInvNumber;
    invInput.readOnly = true;
  }

  // Tampilkan modal menggunakan fungsi modal internal lo (misal: showModal('invoiceModal'))
